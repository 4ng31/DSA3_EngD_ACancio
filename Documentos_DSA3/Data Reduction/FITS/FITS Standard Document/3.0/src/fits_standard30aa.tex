...
\item[\bf Extension] A\index{extension} {\em FITS\/} HDU 
...
\item[\bf {\em\bf FITS} structure] One\index{FITS structure} of the 
...
\item[\bf Mandatory keyword] A\index{keyword, required} keyword that {\em must} 
...
\item[\bf Reserved keyword] An\index{keyword, reserved} {\em optional} keyword 
...
This standard does not impose a limit on the total size\index{file size} of 
...
(approximately $2.1\times10^9$ bytes). 
  ...
  \subsection{Primary header and data unit}
   ...
   the {\tt NAXISn}\index{NAXISn} keyword (Sect.\ \ref{s:pman}). 
\begin{figure}[t]
 \begin{center}
\begin{tabular}{l} \\
          A(1, 1, \ldots, 1), \\ 
...
\end{figure}
   ...
   remainder of the data block {\em shall} be filled\index{fill} by setting all bits to zero.  
   The individual data values {\em shall} be stored \index{byte order} \index{order, byte} in 
...
{\em FITS\/} blocks,\index{blocking} regardless of the physical blocking structure of the
...
\section{Headers}
 ...
 mandatory and reserved keywords for primary arrays and conforming extensions.
  ...
  \label{s:keysyntax}
   ...
   (e.g., by grouping sequences of {\tt COMMENT} keywords\index{COMMENT}
...
See Appendix~H.}  For indexed\index{keyword, indexed}
...
\ \newline \ \newline   If the two ASCII characters \verb*+`= '+ (decimal 61 followed by decimal 32) 
   ...
   (i.e., a {\tt HISTORY},\index{HISTORY} {\tt COMMENT},\index{COMMENT}
...
\ \newline \ \newline   In keyword records that contain the value indicator in bytes~9 and 10, the
   remaining bytes~11 through 80 of the record {\em shall} contain the value,\index{value}
   ...
   The mandatory {\em FITS} keywords\index{keyword, mandatory} defined in this standard
...
trailing spaces are not).  The value of {\tt KEYWORD3} is undefined\index{value, undefined} and has
...
for character-valued keywords.
   ...
   The leading `+' sign\index{sign character} is {\em optional}. 
...
can be represented by a 32-bit or 64-bit signed binary integer).
   ...
   sign\index{sign character} is {\em optional}.
...
Compound units\index{units} strings {\em may} be formed by combining strings of base 
...
% Nevertheless authors might wish to consider whether, e.g., {\tt 'sqrt(erg/(pixel.s.GHz))'} 
% might be more readable than {\tt 'sqrt(erg/pixel/s/GHz)'}. 
...
separated from the\index{slash} slash ({\tt `/'}) comment field delimiter by a
...
% \clearpage
  ...
  \subsection{Keywords}
   ...
   \label{s:keyprime}
\ \newline \ \newline   The {\tt SIMPLE} keyword\index{SIMPLE} is
...
appropriate to the form, range of values, and accuracy of the data in the array.
 ...
 {\em shall} be filled\index{fill} with ASCII spaces (decimal 32 or hexadecimal 20).
...
\ \newline \ \newline   All conforming extensions, whether or not further specified in this standard,
 ...
 in the primary header\index{primary header}.\footnote{This requirement 
...
\end{table}
 ...
 the default interpretations for\index{time} time {\em should} be 
...
\paragraph{}
 ...
 text characters, decimal 32 through 126 (hexadecimal 20 through 7E).
  ...
  This keyword {\em may} be used\index{COMMENT} to supply any comments regarding the {\em FITS\/} file.

 \paragraph{{\tt HISTORY} keyword.}
  This keyword {\em should} be used\index{HISTORY} to describe 
...
\ \newline \ \newline   These keywords are used to describe the contents of an
   ...
   This keyword {\em shall} be \index{BSCALE} used, along with the {\tt BZERO}
...
\end{table*}
 ...
 Since the {\em FITS\/} format does not support a native unsigned\index{integer, unsigned} integer data
 ...
 If the {\tt BSCALE} and {\tt BZERO} keywords\index{BSCALE} do\index{BZERO} not have the
...
\ \newline \ \newline Although these keywords were originally defined for use within\index{extension}
...
The {\em FITS\/} format does not support a native unsigned\index{integer, unsigned} integer
...
keyword\index{BSCALE} {\em shall} have the default value of 1.0 in this case,
...
\label{s:FloatingPoint}
    ...
    format {\em shall} use the\index{ANSI, IEEE} ANSI/IEEE-754 standard \citep{ieee85}.
    {\tt BITPIX~=~-32} and 
    ...
    {\tt -64}; rather, the IEEE NaN\index{NaN, IEEE} {\em should} be used to represent an
    undefined\index{value, undefined} value.  Use of\index{BSCALE} the {\tt BSCALE} and {\tt BZERO}
...
\end{table}
   ...
   The value field {\em shall} contain the integer {\tt 0}, a signature of\index{NAXIS1}
...
be {\verb*+XTENSION= 'IMAGE   '+}.  
   ...
   \subsubsection{Mandatory keywords}
       \label{s:imk}
   The {\tt XTENSION} keyword is required\index{keyword, mandatory} to be the first keyword of 
...
appropriate to the form, range of values, and accuracy of the data in the array.
 ...
 there {\em should not} be any {\tt NAXISn} \index{NAXISn} keywords.
   ...
   \paragraph{{\tt PCOUNT} keyword.}
 The value field {\em shall} contain the integer {\tt 0}.\index{PCOUNT} 
  
   \paragraph{{\tt GCOUNT} keyword.}
 The value field {\em shall} contain the integer {\tt 1}; each\index{GCOUNT}
...
The reserved\index{keyword, reserved} keywords defined in  Sect.\ \ref{s:resk} (except for {\tt EXTEND}
...
be {\verb*+XTENSION= 'TABLE   '+}.  
 ...
 character\index{TABLE} string value text \verb*+'TABLE   '+.
   ...
   \paragraph{{\tt BITPIX} keyword.}
 The value field {\em shall} contain the integer {\tt 8}, denoting\index{BITPIX}
 that the array contains ASCII\index{ASCII character} characters.  
  
   \paragraph{{\tt NAXIS} keyword.}
 The value field {\em shall} contain the integer {\tt 2}, denoting\index{NAXIS}
 ...
 The value field {\em shall} contain the integer {\tt 0}.\index{PCOUNT} 
   ...
   \paragraph{{\tt GCOUNT} keyword.}
 The value field {\em shall} contain the integer {\tt 1}; the data\index{GCOUNT}
...
\end{tabular}
\end{center}
\tablefoot{{\tt w} is the width in characters of the field and
...
\hline
\end{tabular}
\tablefoot{{\tt w} is the width in characters of displayed values, 
...
\end{table*}
 ...
 \label{s:atork}
  In addition to the reserved keywords defined in  Sect.\ \ref{s:resk} (except for {\tt EXTEND}
   and {\tt BLOCKED}), 
   the following other reserved keywords\index{keyword, reserved} {\em may} be 
 ...
 lower case letters, digits, and the\index{underscore} underscore ({\tt `\_'}, decimal 95, 
 ...
 languages (e.g., any hyphens,\index{hyphen} `*'  or `+' characters in the name may be confused
...
following paragraphs.
   ...
   {\tt TBCOL}{\tt n}$ + {\tt w} - 1$ consisting of a\index{sign character} single {\em optional} sign 
...
\end{table*}
   ...
   {\em must} be\index{keyword, mandatory} in the order specified in Table~\ref{t:hdrbin}, with no
 ...
 that follow the table in the supplemental data area called the heap. \index{heap}
   ...
   \paragraph{{\tt GCOUNT} keyword.}
 The value field {\em shall} contain the integer {\tt 1}; the\index{GCOUNT}
 ...
 {\tt n}.  Only the data types in Table~\ref{t:tabBF} are\index{array descriptor} permitted. 
...
\hline
\end{tabular}
\tablefoot{$^{(\dag)}$ Number of 8-bit bytes needed to contain all bits.}
\end{table}
 ...
 in the case of the complex \index{complex data} data types {\tt C} and 
...
(see Sect.\ \ref{s:app_varlen}).
 ...
 integers, 
 the {\tt TZEROn} keyword is also used when storing unsigned\index{integer, unsigned} integer
...
\end{table*}
 ...
 {\tt B}, {\tt I}, {\tt J} or {\tt K},
 or {\tt P} or {\tt Q} array descriptor\index{array descriptor} fields (Sect.\ \ref{s:app_varlen}) that 
...
\end{table*}
   ...
   \paragraph{{\tt TDIMn} keywords.} \label{s:app_dim}
    ...
    the contents of field {\tt n} as a multi-dimensional array\index{array, multi-dimensional}
    ...
    elements {\em should} be interpreted as containing undefined fill\index{fill} values.
...
multi-dimensional\index{array, multi-dimensional} character arrays, {\em may} be represented using the
...
\ \newline \ \newline   The table is constructed from a two-dimensional byte array.
   ...
   \paragraph{Logical.} 
    ...
    of field {\tt n} {\em shall} consist\index{logical value} of ASCII {\tt T} indicating true or  
   ...
   \paragraph{Bit array.} 
    ...
    The most significant byte {\em shall} be\index{byte order}\index{order, byte} first
    ...
    fields must either have a value of 0 (denoting an empty field\index{field, empty} of zero bytes) 
...
\ \newline \ \newline    The main data table {\em may} be followed by a supplemental data area called the heap. \index{heap}
...
array (excluding the type {\tt P} and {\tt Q} variable-length array descriptors\index{array descriptor}
...
approved by the IAUFWG\index{IAUFWG} and therefore are {\em incorporated by reference} as an
...
%% Complete table of WCS keywords Table 22
\begin{table*}
\begin{center}
...
Date of observation & \kwd{MJD-OBS} & \multicolumn{2}{c}{\indxkw{MJDOB}{n}} & \multicolumn{2}{c}{\indxkw{MJDOB}{n}} \\
 ...
 Line rest frequency (Hz) & \kwdalt{RESTFRQ} or  & \multicolumn{2}{c}{\indxkwdalt{RFRQ}{n}} & \multicolumn{2}{c}{\indxkwdalt{RFRQ}{n}} \\
        & \kwd{RESTFREQ}\tablefootmark{4} & \multicolumn{2}{c}{...} & \multicolumn{2}{c}{...} \\
 Line rest vacuum wavelength (m) & \kwdalt{RESTWAV} & \multicolumn{2}{c}{\indxkwdalt{RWAV}{n}} & \multicolumn{2}{c}{\indxkwdalt{RWAV}{n}} \\
...
\end{tabular}
\end{center}
\tablefoot{The indexes {\it j} and {\it i} are pixel and intermediate world coordinate axis numbers, respectively. 
...
\tablefoottext{2}{\indxkw{PC}{i\_j} and \indxkw{CD}{i\_j} forms of the transformation 
...
\tablefoottext{4}{These 8-character keywords are deprecated; the 7-character forms, which can include an alternate version code letter 
...
\item \indxkw{CTYPE}{i} -- [character; indexed; default: {\verb*+' '+} (i.e.\ a
...
specify the coordinate type, the fifth character is a hyphen\index{hyphen} (`{\tt -}'), and the
...
\item \indxkw{CUNIT}{i} -- [character; indexed; default: {\verb*+' '+} (i.e.,
undefined)]. 
Physical units\index{units} of \kwd{CRVAL} and \kwd{CDELT} for axis {\it i}. Note
that units {\em should} always be specified (see Sect.\ \ref{s:Units}). Units for celestial
coordinate systems defined in this Standard {\em must} be degrees. \index{angular units}
...
\item \kwdalt{WCSNAME}  -- [character; default for {\it a}: {\verb*+' '+} (i.e.,
...
hyphen,\index{hyphen} and the next three characters {\em must} specify a predefined algorithm
...
\item \indxkwdalt{CNAME}{i}  -- [character; default: default: {\verb*+' '+}
...
\begin{table*}
\begin{center}
\caption{Reserved spectral coordinate type codes\tablefootmark{1}.}
\label{ta:SPECtype}
\begin{tabular}{lllll} 
\hline \hline
...
{\em shall} be given by the \kwdalt{VELOSYS} keyword.
...
\begin{table}
\centering
\caption{Non-linear spectral algorithm codes\tablefootmark{1}.}
\label{ta:SPECcode}
\begin{tabular}{lll} 
\hline \hline
...
coordinate. Units\index{units} {\em must} be m~s$^{-1}$.  The \indxkwdalt{CUNIT}{i} 
...
{\em FITS\/} free-format\index{format, free} keyword records.\\
...
\subsection{Byte patterns}
 ...
 precision hexadecimal byte\index{NaN, IEEE} patterns. 
...
\bibitem[Note(2000)]{xx_note} {\bf Note:} Many of these {\em FITS\/} references are available electronically 
...
\bibitem[ANSI(1977)]{ansi77} ANSI 1977, {\it American National Standard for Information Processing:
...
\bibitem[Bunclark \& Rots(1997)]{bunclark97} Bunclark, P. \& Rots, A. 1997, {\it Precise re-definition of}
...
\bibitem[Cotton et al.(1990)]{cotton90} Cotton, W. D., et al. 1990, {\it Going AIPS: A Programmer's Guide
to the NRAO Astronomical Image Processing System},  Charlottesville: NRAO

\bibitem[Greisen \& Calabretta(2002)]{greisen02}  Greisen, E. W. \& Calabretta, M. R. 2002,
 A\&A, 395, 1061 

\bibitem[ Greisen et al.(2006)]{greisen06}  Greisen, E. W., Calabretta, M. R., Valdes, F. G., \& Allen, S. L. 2006,
 A\&A, 446, 747

\bibitem[Greisen \& Harten(1981)]{greisen81} Greisen, E. W. \& Harten, R. H. 1981, 
  A\&AS, 44, 371
          
\bibitem[Grosb\o l et al.(1988)]{grosbol88} Grosb\o l, P., Harten, R. H., Greisen, E. W., \& Wells, D. C. 1988, 
 A\&AS, 73, 359 
  
\bibitem[Grosb\o l \& Wells(1994)]{grosbol94} Grosb\o l, P. \& Wells, D. C. 1994, {\it Blocking of Fixed-block
...
\bibitem[IAU(1983)]{iau83} IAU 1983, {\it Transactions of the IAU}, XVIIIB, 45

\bibitem[IAU(1988)]{iau88} IAU\index{IAU} 1988, {\it Transactions of the IAU}, XXB, 51

\bibitem[IEEE(1985)]{ieee85}  IEEE 1985, {\it American National Standard -- 
       IEEE Standard for Binary 
       Floating Point Arithmetic}, ANSI/IEEE 754--1985,
       New York: American
        National Standards Institute, Inc.

\bibitem[ISO(2004)]{iso04} ISO 2004, {\it Information technology -- Programming 
      languages -- Fortran}, \index{Fortran} ISO/ IEC 1539-1:2004, Geneva: 
      International Organization for Standardization
   
\bibitem[McNally(1988)]{mcnally88} McNally, D., ed. 1988, {\it Transactions\index{IAU Style Manual}
...
\bibitem[Wells \& Grosb\o l(1990)]{wells90} Wells, D. C. \& Grosb\o l, P. 1990, {\it Floating Point Agreement
...
\ifiaufwgstyle
  \input fits_standard30aa.ind
\fi

%\typeout{#### To make index, type "makeindex fits_standard_30aa.idx"}
