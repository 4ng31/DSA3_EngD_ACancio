...
for i in `seq -w 0008 10 0358`; do 	#ON
	...
	if [[ -e $FILE1 && -e $FILE2 && -e $FILE3 ]]; then
		python cudadataproc1.py -q 16 -i1 $FILE1 -i2 $FILE2 -i3 $FILE3 -o $OUTFILE >> cuda_resultE1_on1.log ;
	else 
...
for i in `seq -w 0001 10 0361`; do 	#OFF
	...
	FILE3=${BASENAME3}${i}-new
	if [[ -e $FILE1 && -e $FILE2 && -e $FILE3 ]]; then
		python cudadataproc1.py -q 16 -i1 $FILE1 -i2 $FILE2 -i3 $FILE3 -o $OUTFILE >> cuda_resultE1_off1.log ;
	else 
	    echo "File does not exist"
	fi 
done
...
for i in `seq -w 0008 10 0358`; do 	#ON
	...
	FILE3=${BASENAME3}${i}-new
	if [[ -e $FILE1 && -e $FILE2 && -e $FILE3 ]]; then
		python cudadataproc1.py -q 16 -i1 $FILE1 -i2 $FILE2 -i3 $FILE3 -o $OUTFILE >> cuda_resultE2_on1.log ;
	else 
	    echo "File does not exist"
	fi 
done
...
for i in `seq -w 0001 10 0361`; do 	#OFF
	...
	FILE3=${BASENAME3}${i}-new
	if [[ -e $FILE1 && -e $FILE2 && -e $FILE3 ]]; then
		python cudadataproc1.py -q 16 -i1 $FILE1 -i2 $FILE2 -i3 $FILE3 -o $OUTFILE >> cuda_resultE2_off1.log ;
	else 
	    echo "File does not exist"
	fi 
done
